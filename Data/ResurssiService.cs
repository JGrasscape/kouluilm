...
namespace kouluilm.Data
{
    public class ResurssiService
    {
        ...
        public Task<Resurssi> GetResurssiByIDAsync(int resurssi_id)
        {
            // Muuttujat            
            Resurssi resurssi = new Resurssi();
            ...
            using (var connection = new SqliteConnection(connectionStringBuilder.ConnectionString))
            {
                ...
                var sql = connection.CreateCommand();
                sql.CommandText = "SELECT * FROM koulutus_resurssit WHERE resurssi_id=" + resurssi_id;
            ...
            Task<Resurssi> task = Task.FromResult(resurssi);
            return task;
        }
    }
}
